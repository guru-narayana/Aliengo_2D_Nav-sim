...
float jnts_req_pos[12] = {0.0, 0.67, -1.3, -0.0, 0.67, -1.3, 
                      0.0, 0.67, -1.3, -0.0, 0.67, -1.3}; 

void jnt_state_Callback(const unitree_legged_msgs::Aliengo_Joint_controll::ConstPtr msg){
    for(int i=0;i<12;i++){
        jnts_req_pos[i] = msg->joint_positions[i];
        //cout<<msg->joint_positions[i];
    }
}
class multiThread
{
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
    ros::Subscriber jnt_st_sub = n.subscribe("/Aliengo_jnt_req_state", 1, jnt_state_Callback);
    ...
    while (ros::ok()){
        ...
        */
       for(int i=0;i<12;i++){
        lowCmd.motorCmd[i].q = jnts_req_pos[i];
       }
        //cout<<jnts_req_pos[0]<<"\n";
